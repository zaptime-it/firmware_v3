...
double getSupplyAtBlock(std::uint32_t blockNr) {
  ...
  for (int i = 0; i < halvingCount; ++i) {
    totalBitcoinInCirculation +=
        halvingInterval * initialBlockReward * std::pow(0.5, i);
  }

  totalBitcoinInCirculation += (blockNr % halvingInterval) *
                               initialBlockReward * std::pow(0.5, halvingCount);
...
std::string formatNumberWithSuffix(std::uint64_t num, int numCharacters,
  ...
  if (num == 0) {
    if (mowMode) {
      // Preserve existing "divide by million" shape so the "M" suffix still
      // appears.
 ...
 *
 * Based on
 * https://github.com/lnbits/nostr-zap-lamp/blob/main/nostrZapLamp/nostrZapLamp.ino
 */
...
void parseHashrateString(const std::string &hashrate, std::string &label,
                         std::string &output, unsigned int maxCharacters) {
...
int getHashrateMultiplier(char unit) {
  ...
  static const std::unordered_map<char, int> multipliers = {
      {'Z', 21}, {'E', 18}, {'P', 15}, {'T', 12}, {'G', 9}, {'M', 6}, {'K', 3}};
