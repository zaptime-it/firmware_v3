...
void registerOtaRoutes() {
  ...
  server.on("/upload/firmware", AsyncWebRequestMethod::HTTP_POST,
            onFirmwareUpdate, asyncFirmwareUpdateHandler);
  server.on("/upload/webui", AsyncWebRequestMethod::HTTP_POST, onFirmwareUpdate,
            asyncWebuiUpdateHandler);
  server.on("/api/firmware/auto_update", AsyncWebRequestMethod::HTTP_POST,
            onAutoUpdateFirmware);
}
