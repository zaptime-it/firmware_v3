...
EPDManager::EPDManager()
    : currentContent{}, content{}, lastFullRefresh{}, tasks{},
      updateQueue{nullptr}, antonioFonts{nullptr, nullptr, nullptr},
      oswaldFonts{nullptr, nullptr, nullptr}, fontSmall{nullptr},
      fontBig{nullptr}, fontMedium{nullptr}, fontSatsymbol{nullptr},
      bgColor{GxEPD_BLACK}, fgColor{GxEPD_WHITE},
      displays{
#ifdef IS_BTCLOCK_V8
...
void EPDManager::initialize() {
  ...
  xTaskCreate(prepareDisplayUpdateTask, "PrepareUpd", EPD_TASK_STACK_SIZE * 2,
              nullptr, 11, nullptr);

  // Create display update tasks
  for (size_t i = 0; i < NUM_SCREENS; i++) {
    auto *taskParam = new int(i);
    xTaskCreate(updateDisplayTask, ("EpdUpd" + String(i)).c_str(),
                EPD_TASK_STACK_SIZE, taskParam, 11, &tasks[i]);
  }
  ...
  if (mcp1.read1(0) == LOW) {
    ...
    for (size_t i = 0; i < std::min(customText.length(), (size_t)NUM_SCREENS);
         i++) {
...
void EPDManager::loadFonts(const String &fontName) {
  if (fontName == FontNames::ANTONIO) {
    // Load Antonio fonts
    antonioFonts.big =
        FontLoader::loadCompressedFont(Antonio_SemiBold90pt7b_Properties);
    antonioFonts.medium =
        FontLoader::loadCompressedFont(Antonio_SemiBold40pt7b_Properties);
    antonioFonts.small =
        FontLoader::loadCompressedFont(Antonio_SemiBold20pt7b_Properties);
    ...
    // Load Oswald fonts
    oswaldFonts.big =
        FontLoader::loadCompressedFont(Oswald_Medium80pt7b_Properties);
    oswaldFonts.medium =
        FontLoader::loadCompressedFont(Oswald_Medium30pt7b_Properties);
    oswaldFonts.small =
        FontLoader::loadCompressedFont(Oswald_Medium20pt7b_Properties);
  ...
  fontSatsymbol =
      FontLoader::loadCompressedFont(Satoshi_Symbol90pt7b_Properties);
}
...
void EPDManager::setContent(const std::array<String, NUM_SCREENS> &newContent,
                            bool forceUpdate) {
...
void EPDManager::setContent(
    const std::array<std::string, NUM_SCREENS> &newContent) {
...
void EPDManager::splitText(uint dispNum, const String &top,
                           const String &bottom) {
  if (preferences.getBool("verticalDesc", DEFAULT_VERTICAL_DESC) &&
      dispNum == 0) {
...
void EPDManager::showDigit(uint dispNum, char chr, const GFXfont *font) {
  ...
  if (chr == '.') {
    displays[dispNum].fillRect(0, 0, displays[dispNum].width(),
                               round(displays[dispNum].height() * 0.67),
                               bgColor);
  }
}

void EPDManager::showChars(uint dispNum, const String &chars,
                           const GFXfont *font) {
  setupDisplay(dispNum, font);
...
bool EPDManager::renderIcon(uint dispNum, const String &text) {
  ...
  displays[dispNum].drawInvertedBitmap(x_offset, y_offset,
                                       epd_icons_allArray[iconIndex], width,
                                       height, fgColor);
  return true;
}
...
void EPDManager::renderQr(uint dispNum, const String &text) {
  ...
  if (ok) {
    ...
    const int padding =
        floor(float(displays[dispNum].width() - (size * moduleScale)) / 2);
    const int paddingY =
        floor(float(displays[dispNum].height() - (size * moduleScale)) / 2);
    ...
    for (int y = 0; y < size; y++) {
      for (int x = 0; x < size; x++) {
        if (!qrcodegen_getModule(qrcode, x, y))
          continue;
        displays[dispNum].fillRect(padding + x * moduleScale,
                                   paddingY + y * moduleScale, moduleScale,
                                   moduleScale, fgColor);
      }
    }
  }
