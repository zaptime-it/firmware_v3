...
void setupV2Notify() {
  ...
  if (getDataSource() == CUSTOM_SOURCE) {
    ...
    bool useSSL = !preferences.getBool("ceDisableSSL",
                                       DEFAULT_CUSTOM_ENDPOINT_DISABLE_SSL);
...
void onWebsocketV2Event(WStype_t type, uint8_t *payload, size_t length) {
  ...
  case WStype_DISCONNECTED:
    ...
    if (data_source_policy::shouldFlashDataSourceError(disconnectCount,
                                                       WiFi.isConnected())) {
...
void handleV2Message(JsonDocument doc) {
  ...
  } else if (blockFeeDecimals ? doc["blockfee2"].is<float>()
                              : doc["blockfee"].is<float>()) {
    float medianFee = blockFeeDecimals ? doc["blockfee2"].as<float>()
                                       : doc["blockfee"].as<float>();
...
void setupV2NotifyTask() {
  ...
  if (V2Notify::v2NotifyTaskHandle == NULL) {
    xTaskCreate(V2Notify::taskV2Notify, "v2Notify", (6 * 1024), NULL,
                tskIDLE_PRIORITY, &V2Notify::v2NotifyTaskHandle);
  }
}
