...
std::pair<String, uint16_t> splitHostPort(const String &endpoint,
...
void BlockNotify::onWebsocketEvent(WStype_t type, uint8_t *payload,
                                   size_t length) {
...
void BlockNotify::onWebsocketMessage(uint8_t *payload, size_t length) {
  ...
  if (doc["block"].is<JsonObject>()) {
    ...
    if (block["height"].as<uint>() !=
        currentBlockHeight.load(std::memory_order_relaxed)) {
...
void BlockNotify::setup() {
  ...
  if (blockFetch >
      static_cast<int>(currentBlockHeight.load(std::memory_order_relaxed)))
    currentBlockHeight.store(static_cast<uint32_t>(blockFetch),
                             std::memory_order_relaxed);
  if (currentBlockHeight.load(std::memory_order_relaxed) != 0) {
    lastBlockUpdate.store(esp_timer_get_time() / 1000000,
                          std::memory_order_relaxed);
  }
  ...
  if (taskHandle == nullptr) {
    xTaskCreate(BlockNotify::taskBlockNotify, "blockNotify", (6 * 1024),
                nullptr, tskIDLE_PRIORITY, &taskHandle);
  }
}
...
void BlockNotify::processNewBlock(uint32_t newBlockHeight) {
  ...
  if (newBlockHeight - oldBlockHeight > 100) {
    // Old block height is too far behind, update it but don't steal focus or
    // flash LED
  ...
  if (ScreenHandler::getCurrentScreen() != SCREEN_BLOCK_HEIGHT &&
    ...
    vTaskDelay(pdMS_TO_TICKS(
        315 * NUM_SCREENS)); // Extra delay because of screen switching
  }
...
int BlockNotify::fetchLatestBlock() {
  ...
  const String protocol =
      preferences.getBool("mempoolSecure", DEFAULT_MEMPOOL_SECURE) ? "https"
                                                                   : "http";
