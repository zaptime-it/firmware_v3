...
void test_FormatNumberWithSuffix_Zero(void) {
  ...
  // formatNumberWithSuffix(0) must return a sane "0" for both mow and !mow
  // modes.
  ...
  // mowMode falls into the "< thousand" else-branch and divides 0/million =>
  // "0M" (or similar). We only assert non-empty and no crash.
...
void test_FormatNumberWithSuffix_Million(void) {
  ...
  // With numCharacters=4 and "2M" length 2 -> restLen = 4-2-1 = 1 -> "%.1f%c"
  // -> "1.5M"
...
void test_ParseHashrateString_GarbageWithDigit(void) {
  ...
  // Acceptable behaviour: treat as 123 H/S (or fall back to 0). Ensure no
  // throw.
  std::string label, output;
...
void test_GetSupplyAtBlock_Cap(void) {
  ...
  TEST_ASSERT_FLOAT_WITHIN(1.0f, 21000000.0f,
                           (float)getSupplyAtBlock(34 * 210000));
}
