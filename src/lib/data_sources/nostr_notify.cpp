...
/** nostr::ConnectionStatus is CONNECTED=0, DISCONNECTED=1, ERROR=2 — do not
 * index a string array by raw int. */
...
void setupNostrNotify(bool asDatasource, bool zapNotify) {
  ...
  if (asDatasource) {
    ...
    subIdData =
        pool->subscribeMany({relay},
                            ...
                            handleNostrEventCallback, onNostrSubscriptionClosed,
                            onNostrSubscriptionEose);

    if (debugLogEnabled()) {
      Serial.printf("[ Nostr ] debug: data subscription subId=%s relay=%s "
                    "kinds=30078 since=%s author=%s\n",
                    subIdData.c_str(), relay.c_str(),
                    String(getMinutesAgo(60)).c_str(), pubKey.c_str());
    }
  }
  ...
  for (nostr::NostrRelay *r : *relays) {
    ...
    if (r == nullptr || r->getUrl().length() == 0)
      continue;

    r->getConnection()->addConnectionStatusListener(
        [r](const nostr::ConnectionStatus &status) {
          ...
          if (debugLogEnabled()) {
            Serial.printf("[ Nostr ] debug: relay %s connection status=%s\n",
                          r->getUrl().c_str(),
                          nostrConnectionStatusName(status));
          }
        });
  }
}
  ...
  while (1) {
    for (nostr::NostrPool *pool : pools) {
      ...
      if (!nostrIsSubscribed && !nostrIsSubscribing) {
        ...
        subscribeZaps(
            pool, preferences.getString("nostrRelay", DEFAULT_NOSTR_RELAY), 1);
      }
    }
...
boolean nostrConnected() { return nostrIsConnected; }
...
void handleNostrEventCallback(const String &subId,
  ...
  // ws-node publisher format (kind 30078): tag ["d","slot"], content holds the
  // value
  String dTag;
...
void subscribeZaps(nostr::NostrPool *pool, const String &relay,
                   int minutesAgo) {
  if (subIdZap) {
    if (debugLogEnabled()) {
    }
  ...
  subIdZap = pool->subscribeMany(
      ...
      {
          {
              ...
              {"#p",
               {preferences.getString("nostrZapPubkey",
                                      DEFAULT_ZAP_NOTIFY_PUBKEY)}},
              //     {"#p", [&]() {
              //     std::initializer_list<NostrString> pubkeys;
              //     String pubkeysStr =
              //     preferences.getString("nostrZapPubkeys", ""); if
              //     (pubkeysStr.length() > 0) {
              ...
              //         }
              //         free(str);
              //         return std::initializer_list<NostrString>(keys.begin(),
              //         keys.end());
              //     }
      ...
      handleNostrZapCallback, onNostrSubscriptionClosed,
      onNostrSubscriptionEose);
  if (debugLogEnabled()) {
    String zapPubkey =
        preferences.getString("nostrZapPubkey", DEFAULT_ZAP_NOTIFY_PUBKEY);
    Serial.printf("[ Nostr ] debug: zap subscription subId=%s relay=%s "
                  "kinds=9735 since=%s #p=%s\n",
                  subIdZap.c_str(), relay.c_str(),
                  String(getMinutesAgo(minutesAgo)).c_str(), zapPubkey.c_str());
  }
}
...
void handleNostrZapCallback(const String &subId,
  ...
  std::array<std::string, NUM_SCREENS> textEpdContent = parseZapNotify(
      zapAmount, preferences.getBool("useSatsSymbol", DEFAULT_USE_SATS_SYMBOL));
  ...
  } else if (preferences.getBool("scrnRestoreZap",
                                 DEFAULT_SCREEN_RESTORE_AFTER_ZAP)) {
    TimerHandle_t screenRestoreAfterZapTimer = xTimerCreate(
        "screenRestoreAfterZap", pdMS_TO_TICKS(getTimerSeconds() * msPerSecond),
        pdFALSE, (void *)(uintptr_t)screenBeforeZap,
...
//         if (amount > 0) {
//             std::array<std::string, NUM_SCREENS> zapContent =
//             parseZapNotify(amount, true);
//             EPDManager::getInstance().setContent(zapContent);

//             if (preferences.getBool("ledFlashOnUpd",
//             DEFAULT_LED_FLASH_ON_UPD)) {
