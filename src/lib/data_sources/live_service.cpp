...
// Matches the prior bespoke 5-minute threshold in handlePriceNotifyDisconnection
// and handleBlockNotifyDisconnection.
...
void LiveServiceRegistry::clear() {
    entries.clear();
}
    ...
    for (auto& entry : entries) {
        ...
        if (!service->isConnected()) {
            if (entry.disconnectedSince == 0) {
                ...
                Serial.printf("[ LiveService ] %s disconnected, watching for reconnect...\r\n",
                              service->name());
            } else if ((now - entry.disconnectedSince) > kDisconnectRestartSeconds) {
                Serial.printf("[ LiveService ] %s still disconnected after %lus, restarting...\r\n",
        ...
        if (staleWindow > 0 && lastUpdate != 0 &&
            now > lastUpdate && (now - lastUpdate) > staleWindow) {
            Serial.printf("[ LiveService ] %s stale for %lus (> %lus), restarting...\r\n",
