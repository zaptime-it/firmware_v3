...
PoolStats BraiinsPool::parseResponse(const JsonDocument &doc) const {
  if (doc["btc"].isNull()) {
    return PoolStats{.hashrate = "0", .dailyEarnings = 0};
  }
  ...
  return PoolStats{.hashrate =
                       std::to_string(static_cast<int>(std::round(hashValue))) +
                       std::string(multiplier, '0'),
                   .dailyEarnings = static_cast<int64_t>(
                       doc["btc"]["today_reward"].as<float>() * 100000000)};
}
