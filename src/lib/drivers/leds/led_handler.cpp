...
LedHandler::LedHandler()
    : pixels(NEOPIXEL_COUNT, NEOPIXEL_PIN, NEO_GRB + NEO_KHZ800),
      ledTaskHandle(nullptr), ledTaskQueue(nullptr), ledTaskParams(0),
      dndEnabled(false), dndTimeBasedEnabled(false),
      dndTimeRange{23, 0, 7, 0} // Default: 23:00 to 07:00
#ifdef HAS_FRONTLIGHT
      ,
      frontlightOn(false), flInTransition(false)
#endif
{
}
...
void LedHandler::ledTask(void *pvParameters) {
  ...
  while (true) {
    if (handler->ledTaskQueue != nullptr) {
      if (xQueueReceive(handler->ledTaskQueue, &handler->ledTaskParams,
        ...
        case LED_POWER_TEST:
#ifdef HAS_FRONTLIGHT
          handler->frontlightFadeInAll(preferences.getUInt("flEffectDelay"),
                                       true);
#endif
          handler->rainbow(20);
        ...
        case LED_EFFECT_WIFI_CONNECT_ERROR:
          handler->blinkDelayTwoColor(100, 3,
                                      handler->pixels.Color(8, 161, 236),
        ...
        case LED_EFFECT_WIFI_WAIT_FOR_CONFIG:
          handler->blinkDelayTwoColor(100, 1,
                                      handler->pixels.Color(8, 161, 236),
        ...
        case LED_EFFECT_WIFI_CONNECTING:
          ...
          for (int i = NEOPIXEL_COUNT - 1; i >= 0; i--) {
            for (int j = NEOPIXEL_COUNT - 1; j >= 0; j--) {
              if (j == i) {
                handler->pixels.setPixelColor(
                    i, handler->pixels.Color(16, 197, 236));
              } else {
                handler->pixels.setPixelColor(j,
                                              handler->pixels.Color(0, 0, 0));
              }
            }
            handler->pixels.show();
        ...
        case LED_EFFECT_START_TIMER:
          handler->pixels.clear();
          handler->pixels.setPixelColor((NEOPIXEL_COUNT - 1),
                                        handler->pixels.Color(255, 0, 0));
          handler->pixels.show();
          delay(900);
...
void LedHandler::blinkDelayTwoColor(int d, int times, const uint32_t &c1,
                                    const uint32_t &c2) {
  for (int j = 0; j < times; j++) {
    for (int i = 0; i < NEOPIXEL_COUNT; i++) {
...
void LedHandler::setDNDTimeRange(uint8_t startHour, uint8_t startMinute,
...
bool LedHandler::isTimeInDNDRange(uint8_t hour, uint8_t minute) const {
  ...
  return btclock::isTimeInDNDRange(
      hour, minute, dndTimeRange.startHour, dndTimeRange.startMinute,
      dndTimeRange.endHour, dndTimeRange.endMinute);
}
...
void LedHandler::frontlightFadeInAll(int flDelayTime, bool staggered) {
  ...
  if (staggered) {
    ...
    for (int dutyCycle = 0;
         dutyCycle <=
         maxBrightness + (NUM_SCREENS - 1) * maxBrightness / NUM_SCREENS;
         dutyCycle += step) {
      for (int ledPin = 0; ledPin < NUM_SCREENS; ledPin++) {
    ...
    for (int dutyCycle = 0; dutyCycle <= maxBrightness;
         dutyCycle += FL_FADE_STEP) {
...
void LedHandler::frontlightFadeOutAll(int flDelayTime, bool staggered) {
  ...
  if (staggered) {
    ...
    for (int dutyCycle = maxBrightness; dutyCycle >= 0; dutyCycle -= step) {
      for (int ledPin = 0; ledPin < NUM_SCREENS; ledPin++) {
        int ledBrightness = dutyCycle - (NUM_SCREENS - 1 - ledPin) *
                                            maxBrightness / NUM_SCREENS;
        if (ledBrightness < 0) {
          ledBrightness = 0;
        } else if (ledBrightness > maxBrightness) {
          ledBrightness = maxBrightness;
    ...
    for (int dutyCycle = preferences.getUInt("flMaxBrightness"); dutyCycle >= 0;
         dutyCycle -= FL_FADE_STEP) {
      for (int ledPin = 0; ledPin < NUM_SCREENS; ledPin++) {
        flArray.setPWM(ledPin + 1, 0, dutyCycle);
      }
...
void LedHandler::frontlightFadeIn(uint num, int flDelayTime) {
  ...
  for (int dutyCycle = 0; dutyCycle <= preferences.getUInt("flMaxBrightness");
       dutyCycle += 5) {
...
void LedHandler::initializeFrontlight() {
  if (!flArray.begin(PCA9685_MODE1_AUTOINCR | PCA9685_MODE1_ALLCALL,
                     PCA9685_MODE2_TOTEMPOLE)) {
