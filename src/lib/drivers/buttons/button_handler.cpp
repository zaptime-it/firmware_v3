    ...
    if (intAsserted) {
      ...
      for (int i = 0; i < 4; i++) {
        if (buttonStates[i].isPressed) {
          ...
          switch (i) {
          case 0:
            currentlyPressed = (intCap & BTN_1);
            break;
          case 1:
            currentlyPressed = (intCap & BTN_2);
            break;
          case 2:
            currentlyPressed = (intCap & BTN_3);
            break;
          case 3:
            currentlyPressed = (intCap & BTN_4);
            break;
          }
          if (!currentlyPressed) {
...
void ButtonHandler::handleButtonRelease(int buttonIndex) {
  ...
  if (!state.isPressed)
    return; // Ignore if button wasn't pressed
