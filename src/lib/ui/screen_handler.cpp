...
bool ScreenHandler::handleCurrencyRotation(bool forward) {
  if ((getDataSource() == BTCLOCK_SOURCE || getDataSource() == CUSTOM_SOURCE) &&
    ...
    if (it == ac.end()) {
      // Current currency not found in active currencies - initialize based on
      // direction
      ...
      setCurrentCurrency(
          getCurrencyChar(ac.at(std::distance(ac.begin(), it) + 1)));
      setCurrentScreen(getCurrentScreen());
      return true;
    ...
    // If we're at the last/first currency of current screen, let
    // nextScreen/previousScreen handle it
    return false;
  }
  return false;
}
...
int ScreenHandler::findNextVisibleScreen(int currentScreen, bool forward) {
  ...
  if (currentScreen < 0 ||
      static_cast<size_t>(currentScreen) >= screenMappings.size()) {
    currentScreen = 0;
  }
  ...
  if (forward) {
    newScreen = (static_cast<size_t>(currentScreen) < screenMappings.size() - 1)
                    ? screenMappings[currentScreen + 1].value
                    : screenMappings.front().value;
  } else {
    newScreen = (currentScreen > 0) ? screenMappings[currentScreen - 1].value
                                    : screenMappings.back().value;
  }
  ...
  while (!preferences.getBool(key.c_str(), true)) {
    ...
    if (forward) {
      newScreen =
          (static_cast<size_t>(currentScreen) < screenMappings.size() - 1)
              ? screenMappings[currentScreen + 1].value
              : screenMappings.front().value;
    } else {
      newScreen = (currentScreen > 0) ? screenMappings[currentScreen - 1].value
...
void ScreenHandler::nextScreen() {
  if (handleCurrencyRotation(true))
    return;
...
void ScreenHandler::previousScreen() {
  if (handleCurrencyRotation(false))
    return;
...
void ScreenHandler::resetCurrencyForScreen(uint targetScreen, bool forward) {
  ...
  int idx = btclock::nextCurrencyIndex(isCurrencySpecific(targetScreen),
                                       forward, static_cast<int>(ac.size()));
  if (idx < 0)
    return;
    ...
    if (xQueueReceive(workQueue, &receivedItem, portMAX_DELAY)) {
      ...
      case TASK_BITAXE_UPDATE: {
        if (currentScreenValue != SCREEN_BITAXE_HASHRATE &&
            currentScreenValue != SCREEN_BITAXE_BESTDIFF)
          break;

        taskEpdContent =
            (currentScreenValue == SCREEN_BITAXE_HASHRATE)
      ...
      case TASK_MINING_POOL_STATS_UPDATE: {
        if (currentScreenValue != SCREEN_MINING_POOL_STATS_HASHRATE &&
            currentScreenValue != SCREEN_MINING_POOL_STATS_EARNINGS)
          break;
        ...
        taskEpdContent =
            (currentScreenValue == SCREEN_MINING_POOL_STATS_HASHRATE)
                ? parseMiningPoolStatsHashRate(miningFetch.getHashRate(), *pool)
                : parseMiningPoolStatsDailyEarnings(
                      miningFetch.getDailyEarnings(),
                      pool->getDailyEarningsLabel(), *pool);
        EPDManager::getInstance().setContent(taskEpdContent);
        break;
      }
      ...
      case TASK_PRICE_UPDATE: {
        ...
        if (currentScreenValue == SCREEN_BTC_TICKER) {
          ...
          taskEpdContent = parseMarketCap(
              blockNotify.getBlockHeight(), price, currency,
      ...
      case TASK_BLOCK_UPDATE: {
        if (currentScreenValue == SCREEN_BITCOIN_SUPPLY) {
          auto &blockNotify = BlockNotify::getInstance();
          taskEpdContent = parseBitcoinSupply(
              blockNotify.getBlockHeight(),
              preferences.getBool("mcapBigChar", DEFAULT_MCAP_BIG_CHAR),
              preferences.getBool("supplyPercent", DEFAULT_SUPPLY_PERCENT));
        } else if (currentScreenValue == SCREEN_HALVING_COUNTDOWN) {
          auto &blockNotify = BlockNotify::getInstance();
          taskEpdContent = parseHalvingCountdown(
              blockNotify.getBlockHeight(),
              preferences.getBool("useBlkCountdown",
                                  DEFAULT_USE_BLOCK_COUNTDOWN));
        } else {
          auto &blockNotify = BlockNotify::getInstance();
...
void setupTasks() {
  ...
  if (findScreenIndexByValue(
          preferences.getUInt("currentScreen", DEFAULT_CURRENT_SCREEN)) != -1)
    ScreenHandler::setCurrentScreen(
        preferences.getUInt("currentScreen", DEFAULT_CURRENT_SCREEN));
}
