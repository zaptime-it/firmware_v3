...
static void onApiLightsSetJson(AsyncWebServerRequest *request,
                               JsonVariant &json) {
  if (requireHttpAuth(request))
    return;
...
static void onApiFrontlightSetBrightness(AsyncWebServerRequest *request) {
  ...
  getLedHandler().frontlightSetBrightness(
      request->getParam("b")->value().toInt());
  request->send(HTTP_OK);
  notifyEventSourceStatus();
}
...
void registerLightsRoutes() {
  ...
  server.on("/api/frontlight/brightness", HTTP_POST,
