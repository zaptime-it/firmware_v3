...
static void onApiScreenControl(AsyncWebServerRequest *request)
{
  ...
  if (action.endsWith("/next"))           ScreenHandler::nextScreen();
  else if (action.endsWith("/previous"))  ScreenHandler::previousScreen();
  request->send(HTTP_OK);
  notifyEventSourceStatus();
}
...
static void onApiShowTextAdvanced(AsyncWebServerRequest *request, JsonVariant &json)
{
  if (requireHttpAuth(request)) return;
...
void registerActionRoutes()
{
  ...
  server.addRewrite(
      new OneParamRewrite("/api/show/number/{number}", "/api/show/text?t={number}"));
}
