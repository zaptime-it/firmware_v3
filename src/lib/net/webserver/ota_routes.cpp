...
static void onFirmwareUpdate(AsyncWebServerRequest *request)
{
  ...
  AsyncWebServerResponse *response =
      request->beginResponse(HTTP_OK, "text/plain", shouldReboot ? "OK" : "FAIL");
...
static void asyncFileUpdateHandler(AsyncWebServerRequest *request, String filename,
                                   ...
                                   bool final, int command)
{
  ...
  if (!index)
  {
    ...
    else if (command == U_SPIFFS)
    {
      ...
      if (fsPart) fsSize = fsPart->size & ~(SPI_FLASH_SEC_SIZE - 1);
      s_fsCapSize = fsSize;
      s_fsWritten = 0;
...
static void asyncWebuiUpdateHandler(AsyncWebServerRequest *request, String filename,
                                    size_t index, uint8_t *data, size_t len,
                                    bool final)
{
...
static void asyncFirmwareUpdateHandler(AsyncWebServerRequest *request, String filename,
                                       size_t index, uint8_t *data, size_t len,
                                       bool final)
{
  if (index == 0 && requireHttpAuth(request)) return;
...
void registerOtaRoutes()
{
  ...
  if (!preferences.getBool("otaEnabled", DEFAULT_OTA_ENABLED)) return;
